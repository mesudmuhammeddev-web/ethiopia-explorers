...
import { getCategoryTour, getCategory, includedDefault, excludedDefault } from "@/data/experiences";
...
const ExperienceTour = () => {
  ...
  const tour = catSlug && slug ? getCategoryTour(catSlug, slug) : undefined;

  if (!category || !tour) return <Navigate to="/" replace />;

  const included = tour.included ?? includedDefault;
  const excluded = tour.excluded ?? excludedDefault;
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        ...
        canonicalPath={`/experiences/${category.slug}/${tour.slug}`}
      />
      <Navbar />
        ...
        <div className="relative h-[55vh] min-h-[400px] overflow-hidden bg-muted">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12">
            <div className="container mx-auto">
              <Link to={`/experiences/${category.slug}`} className="inline-flex items-center gap-2 text-white/90 hover:text-white text-sm mb-4">
                <ArrowLeft className="w-4 h-4" /> All {category.label} Tours
              </Link>
